...
def test_lock_manager_context(mock_redis, lock_config):
    ...
    with JobOperationLock(job_hash=job_hash, operation=JobOperation.START, **lock_config) as lock:
        # test lock acquired
        assert lock
        # verify lock key in cache
        assert f"lock_{JobOperation.START.value}_{job_hash}" in mock_redis_connection.cache
    # verify release
    assert f"lock_{JobOperation.START.value}_{job_hash}" not in mock_redis_connection.cache
